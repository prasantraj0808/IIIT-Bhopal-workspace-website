...
if(isset($_GET['show_fac'])){
	
	echo "<div id=\"about-para\">";

        echo "<p>  <span class=\"text-highlight\">All Registerd Faculties</span> <br><br>";
	$m=mysqli_query($con,"SELECT * from reg_fac");
	if($m)
	{   $i=0;
		while($x=mysqli_fetch_assoc($m))
			...
			echo " <br><a href=\"registration_fac.php\"><input type=\"button\" value=\"Back\"></a> <hr>";
	}
	else
	{
	echo "Oops!Error to show all registration";
	}
...
echo "<hr> <h3> Details of faculty going to be Registerd. </h3>";	
echo "<br> name : {$name} <br> Email : {$email}  <br> Time : {$time}  <br> IP : {$ip} "; 

	$temp="SELECT * from reg_fac where email='{$email}' ";
	$m=mysqli_query($con,$temp);
	$aa = mysqli_num_rows($m);
	if($aa>=1)
	{
    	echo "<br><h3>Your email should be changed. This email [ {$email} ]  is used by another member of the website.</h3><br> <a href=\"registration_fac.php\"><input type=\"button\" value=\"Back\"></a>";
	}
	else
	{	
$n=mysqli_query($con,"insert into reg_fac(name,email,phone,addr,dept,time,ip) values('{$name}','{$email}','{$phone}','{$addr}','{$dept}','{$time}','{$ip}')");
	    if($n)
        { echo"<h4> <b style=\"color:green;\"> Your registration is successful. login_ID is: {$email} and Password is: {$phone}</b></h4> <a href=\"registration_fac.php\"><input type=\"button\" value=\"Back\"></a>  ";
		}
		else { echo"<h4> <b style=\"color:green;\"> Your registration is not successful.<br> Try again 
		<a href=\"registration_fac.php\"><input type=\"button\" value=\"Back\"></a> ";
		}
	}
}
...
{
echo "<hr> <h4 style=\"color:green;font-weight:400;border-bottom:2px dotted red;font-style:italic;\"> Faculty Registration</h4>";		
echo "<form style=\"font-size:10px;font-weight:200;\" method=\"post\" action=\"registration_fac.php\">";
echo "Name :&nbsp; &nbsp; <input name=\"fac_name\" type=\"text\" required  /> <br><br>";
echo "Email :&nbsp; &nbsp; <input name=\"fac_email\" type=\"text\" required  /> <br><br>";
echo "Phone : &nbsp; &nbsp;<input name=\"fac_phone\" type=\"text\" required  /> <br><br>";
echo "Department : <select name=\"fac_dept\" required >";
...
echo" </select>  </br><br>";
echo "Address : <input name=\"fac_addr\" type=\"text\" required  /> <br><br>";
echo "<input name=\"fac_submit\" type=\"submit\" value=\"Register\" /> <br><br>";
echo " </form> ";
